...
def test_database_integration_save_and_load(monkeypatch):
    ...
    test_token_pos = 10
    # Attempt to save and then load. If the team's database functions behave differently this will adapt.
    try:
        database.save_player_stats(conn, type("P", (), {"name": test_name, "token_position": test_token_pos, "tokens_finished": 0}))
    except TypeError:
        # alternative signature support: save_player_stats(conn, name, token_position, tokens_finished)
        try:
